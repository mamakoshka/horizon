...
/client/proc/cmd_admin_pm(whom, msg)
	...
	if(external)
		...
		if(recipient.holder && !badmin)
			if(holder)
				to_chat(recipient,
					type = MESSAGE_TYPE_ADMINPM,
					html = SPAN_DANGER("Admin PM from-<b>[key_name_pm(src, recipient, 1)]</b>: <span class='linkify'>[keywordparsedmsg]</span>"),
					confidential = TRUE)
				to_chat(src,
					type = MESSAGE_TYPE_ADMINPM,
					html = SPAN_NOTICE("Admin PM to-<b>[key_name_pm(recipient, src, 1)]</b>: <span class='linkify'>[keywordparsedmsg]</span>"),
					confidential = TRUE)
			...
			if(holder) //sender is an admin but recipient is not. Do BIG RED TEXT
				...
				to_chat(recipient,
					type = MESSAGE_TYPE_ADMINPM,
					html = SPAN_ADMINSAY("Admin PM from-<b>[key_name_pm(src, recipient, 0)]</b>: <span class='linkify'>[msg]</span>"),
					confidential = TRUE)
				to_chat(recipient,
					type = MESSAGE_TYPE_ADMINPM,
				...
				to_chat(src,
					type = MESSAGE_TYPE_ADMINPM,
					html = SPAN_NOTICE("Admin PM to-<b>[key_name_pm(recipient, src, 1)]</b>: <span class='linkify'>[msg]</span>"),
					confidential = TRUE)
	...
	if(external)
		...
		for(var/client/X in GLOB.admins)
			to_chat(X,
				type = MESSAGE_TYPE_ADMINPM,
				html = SPAN_NOTICE("<B>PM: [key_name_pm(src, X, 0)]-&gt;External:</B> [keywordparsedmsg]"),
				confidential = TRUE)
	else
		...
		for(var/client/X in GLOB.admins)
			if(X.key!=key && X.key!=recipient.key) //check client/X is an admin and isn't the sender or recipient
				to_chat(X,
					type = MESSAGE_TYPE_ADMINPM,
					html = SPAN_NOTICE("<B>PM: [key_name_pm(src, X, 0)]-&gt;[key_name_pm(recipient, X, 0)]:</B> [keywordparsedmsg]"),
					confidential = TRUE)
